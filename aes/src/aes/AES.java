...
class AES {
	...
	ROUNDS = 10,        // AES has 10-14 rounds
	...
	static {
		int i, j;
		// produce tables, needed for multiplying in the field GF(2^8)
		alog[0] = 1;
		for (i = 1; i < 256; i++) {
	...
	//......................................................................
	/*** AES encrypt 128-bit plain text using key previously set. */
	public byte[] encrypt(byte[] plain) {
		...
		// copy plain text bytes into state and do initial AddRoundKey(state)
		Ker = Ke[0];
		...
		for (int r = 1; r < numRounds; r++) {
			...
			//System.out.println("Column byte:" +Util.toHEX1(ta));
	...
	//......................................................................
	/** * AES decrypt 128-bit cipher text using key previously set.*/
...
*/
	...
	public String _cryptAll(String data, int mode)  {
		...
		byte[] res1=new byte[data.length()];
		String partStr = "";
		...
		for(int p=0; p<nParts; p++) {
			...
			//cipher=static_stringToByteArray(partStr);
			if(mode==1){ 					//mac
				//System.out.println(nParts);
				for(int r=0;r<numRounds;r++){
				if (Roundone==0){
					for(int k=0;k<partByte.length;k++)input[k]=(byte)(partByte[k]^iv[k]);
					cipher= aes.encrypt(input);
				}
				else{
					for(int k=0;k<partByte.length;k++)input[k]=(byte)(partByte[k]^cipher[k]);
					cipher=aes.encrypt(input);
			...
			if(mode==2){partByte = aes.decrypt(partByte);}//aes.decrypt(partByte);
			for(int b=0; b<16; b++){
				res[p*16+b] = partByte[b];
				res1[p*16+b]=cipher[b];
			}
		}
		
		mac=static_byteArrayToString(res1);
		String hex1 = Util.toHEX(mac.getBytes()).replace(" ", "");
		mac=hex1;
	...
	public String getMAC(){
		String mac_code=mac;
		//System.out.println("MAC code: "+mac_code);
		return mac_code;
	}
